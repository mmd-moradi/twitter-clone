...
const EditModal = () => {
  ...
  const onSubmit = useCallback(async () => {
    try {
      ...
      await axios.post("/api/edit", { name, username, bio, profileImage, coverImage });
      mutateFetchedUser();
  ...
  const bodyContent = (
    <div className="flex flex-col gap-4">
      <ImageUpload value={profileImage} disabled={isLoading}
      onChange={(image) => setProfileImage(image)} label="Upload profile image" />
      <ImageUpload value={coverImage} disabled={isLoading}
